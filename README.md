...
At v4.0.0, BMsql provides a library crate with an in-memory `Database` type, a `BmsqlError` type, a fixed-size `Page` abstraction (4096 bytes), page offset calculation, and a `DatabaseFile` storage type that opens a file and reads/writes pages by ID at their file offset. The CLI prints the database name. There is no row storage or SQL yet.
