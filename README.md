...
At v0.4.0, BMsql builds on the pager with structured page layout and basic record insertion: a 7-byte `PageHeader`, a `Slot` type, and `Page::insert_record`. The `Pager` still provides a bounded FIFO page cache, page I/O, size, page count, and allocation. The CLI prints the database name. There is no record scan API, tables/schemas, or SQL yet.
...
│   ├── page.rs                 # Page, PageHeader, Slot; insert_record
